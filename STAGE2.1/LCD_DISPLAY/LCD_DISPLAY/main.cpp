...
int main(void)
{
	...
	DDRB = 0x07;                                               //0x07=0b00000111 set d0,d1,d2 as output pins(LED"s are connected)
	PORTB=0x38;                                                //0x38=0b00111000 set d3,d4,d5 as high so it can read the value(button's)
	
    while (1) 
	
	{
		if(!(PINB & 0x08))                                     //checks whether the button of d3 is pressed
		{
			PORTB |= 0x01;                                     //0x01=0b00000001, make d0 as high
		...
		{
			PORTB &= 0xFE;                                     //make d0 as low
		}
		
		if(!(PINB & 0x10))                                     //checks whether the button of d4 is pressed
		{
			PORTB |= 0x02;                                     //0x02=0b00000010, make d1 as high
			lcd.LCD_String("YOU HAVE PRESSED");                //Write string on 1st line of LCD
			lcd.LCD_Commandgiver(0xC0);                        //Go to 2nd line
		...
		{
			PORTB &= 0xFD;                                     //make d1 as low
		}
		
		if(!(PINB & 0x20))                                     //checks whether the button of d5 is pressed
		{
			PORTB |= 0x04;                                     //0x04=0b00000100, make d2 as high
			lcd.LCD_String("YOU HAVE PRESSED");                //Write string on 1st line of LCD
			lcd.LCD_Commandgiver(0xC0);                        //Go to 2nd line
